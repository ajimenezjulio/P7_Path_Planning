...
int main() {
  ...
  /**
    * VARIABLES TO SMOOTH THE MOVEMENT
    */
  
  // Starting lane, 0 = left, 1 = mid, 2 = right
  int lane = 1;
  // Reference velocity to target (should be less than limit, < 50mph)
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          if (prev_size > 0) {
            // If there are points, set `car_s` to previous path
            // last point s. We will work in Frenet
            car_s = end_path_s;
          }
          
          vector<double> predictions = predictions_from_sensor_fusion(sensor_fusion, prev_size, car_s, lane);
          
          // Flags for presence of cars close to us in the 3 different lanes
          bool car_left = predictions[0];
          bool car_right = predictions[1];
          bool car_front = predictions[2];
          // Speed of the car in front of us, if any
          double front_speed = predictions[3];
          
          // Get list of possible moves/trajectories (each move has a target lane and target speed)
          vector<std::pair<int, double>> successors = behaviour_planning(car_left, car_right, car_front, lane, ref_vel, front_speed);
          
          // Choose the least cost trajectory
          double lowest_cost = INT_MAX;
          
          for(int i = 0; i < successors.size(); i++) {
            // Get target lane and speed of current possible move
